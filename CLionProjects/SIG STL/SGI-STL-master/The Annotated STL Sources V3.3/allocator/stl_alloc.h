...
//如果编译器无法处理static member of template classes(模板类静态成员)就定义__STL_STATIC_TEMPLATE_MEMBER_BUG
#ifndef __STL_STATIC_TEMPLATE_MEMBER_BUG
// 初值为0，由客户自行设定
template <int __inst>
...
__malloc_alloc_template<__inst>::_S_oom_malloc(size_t __n)
{
    ...
    for (;;) {
        ...
        if (0 == __my_malloc_handler) { __THROW_BAD_ALLOC; }   //__THROW_BAD_ALLOC就是打印申请失败信息然后退出
